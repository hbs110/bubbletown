...
cocos2d::Node* BtGui_utouch::LoadLayout(const std::string& layoutFile)
{
    ...
    cocos2d::Vec2 runtimeRes = cocos2d::Vec2(cocos2d::Director::getInstance()->getVisibleSize());
    BtUINodeBuilder_utouch builder(designRes, runtimeRes);

    auto node = LoadLayoutRecursively(&builder, doc);
    BT_EXPECT_RET_V2(node, tfm::format("layout file parsing (recursively) failed. ('%s')", layoutFilename), nullptr);
    return node;
}

cocos2d::Node* BtGui_utouch::LoadLayoutRecursively(BtUINodeBuilder_utouch* builder, const rapidjson::Value& jsonNode)
{
    ...
    auto node = builder->Create(ctrlType, jsonNode);
